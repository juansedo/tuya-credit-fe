...
export const getAnnuityFees = (feesNumber: number, interestRate: number, totalAmount: number, handlingFee: number) => {
  let annuity =
    totalAmount *
    ((Math.pow(1 + interestRate, feesNumber) * interestRate) / (Math.pow(1 + interestRate, feesNumber) - 1));
...
export const getAnnuityValue = (feesNumber: number, interestRate: number, totalAmount: number) => {
  let annuity =
    totalAmount *
    ((Math.pow(1 + interestRate, feesNumber) * interestRate) / (Math.pow(1 + interestRate, feesNumber) - 1));
...
export const getDecreasingValues = (feesNumber: number, interestRate: number, totalAmount: number) => {
  const minium: number = totalAmount / feesNumber;
  let maxium: number;
  if (feesNumber > 1) {
    maxium = minium + totalAmount * interestRate + (totalAmount - minium) * interestRate;
  } else {
    maxium = minium;
  }
...
export const getAnnuityTotal = (feesNumber: number, interestRate: number, totalAmount: number, handlingFee: number) => {
  const annuity =
    totalAmount *
    ((Math.pow(1 + interestRate, feesNumber) * interestRate) / (Math.pow(1 + interestRate, feesNumber) - 1));
...
export const getDecreasingTotal = (
  ...
  total += amortizationFee + totalAmount * interestRate + (totalAmount - amortizationFee) * interestRate;
